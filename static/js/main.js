...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const selectedLanguage = languageSelect ? languageSelect.value : 'en';  // Default to English if selector not found
    ...
    function handleFileSelected(e) {
        ...
        if (isImage) {
            ...
            reader.onload = function(evt) {
                const imgHtml = `<div class="attachment attachment-image"><img src="${evt.target.result}" alt="attachment" style="max-width:220px;border-radius:10px;" /></div>`;
            ...
            const chip = `<span class="file-chip"><i class="fas fa-file"></i> ${safeName}</span>`;
            addMessage(chip, true);
    ...
    // Send to server with language info
    fetch('/chat', {
        method: 'POST',
    ...
    .then(data => {
        ...
        if (data.language && data.language !== selectedLanguage) {
            ...
            showToast(`Language switched to ${getLanguageName(data.language)}`);
        }
